...
Each observation in this data set represents a state, including DC.
...
***There are `r nrow(dennys)` rows and `r ncol(dennys)` columns. Each row represents the data for one denny's resturant. The variables are: `r names(dennys)`.***

2.  What are the dimensions of the La Quinta's dataset?
    What does each row in the dataset represent?
    What are the variables?
    
***There are `r nrow(laquinta)` rows and `r ncol(laquinta)` columns. Each row represents the data for one La Quintas Inn. The variables are: `r names(laquinta)`.***
...
3.  Take a look at the websites that the data come from (linked above).Are there any La Quinta's locations outside of the US?
If so, which countries?
What about Denny's?

***La Quintas hotels have locations mostly in the Americas but also have some locations in other countries. Denny's are located in the US.***
...
***We can use the State variable to determine if a location is in the US. Specifically,  by comparing the state listed to a database of states in the US.***
...
***There are no Denny's locations outside the US in our data.***
...
***Delaware has the fewest Denny's locations and California has the most locations. For La Quinta, Texas has the most locations and Maine has the fewest.***
...
***For North Carolina, there are very few Denny's and La Quinta locations next to each other, so the joke does not hold up in this state***
...
***For Texas, there are many La Quinta and Denny's locations next to each other, so the joke holds up in this state.***
